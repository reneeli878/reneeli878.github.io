...
function updateTodayDashboard(records) {
  ...
  const clockInRecords = todayRecords
    .filter((record) => record.action === '已上班')
  ...
  dashboard.value = {
    ...
    distance:
      latestRecord.distance !== '' && latestRecord.distance !== undefined
        ...
        : '-- m',
    firstClockIn: clockInRecords.length ? formatTimeOnly(clockInRecords[0].timestamp) : '--',
    lastUpdate: `${formatTimeOnly(latestRecord.timestamp)} ${latestRecord.action || ''}`.trim()
  }
