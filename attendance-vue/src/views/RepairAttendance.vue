<template>
  ...
  >
    ...
    >
      <div
        class="mx-auto flex min-h-[66px] w-[min(1120px,calc(100%-28px))] items-center justify-between gap-3 max-sm:w-[min(100%-16px,1120px)]"
      >
    ...
    <main class="py-4 max-sm:py-3">
      <div
        class="mx-auto w-[min(1120px,calc(100%-28px))] max-sm:w-[min(100%-16px,1120px)]"
      >
        <section
        ...
        >
          <div class="mb-4">
            ...
            <p class="mt-1 text-sm text-slate-500">
              送出後會寫入 Google Sheet，並同步顯示在下方申請紀錄。
            </p>
          </div>

          <div
            class="mb-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600"
          >
            <div>
              <strong>申請人：</strong
              >{{ currentUser.employeeName || currentUser.name || "未登入" }}
            </div>
            <div class="mt-1">
              <strong>員工編號：</strong>{{ currentUser.employeeCode || "--" }}
            </div>
            <div class="mt-1">
              <strong>部門職稱：</strong>{{ currentUser.dept || "尚未建檔" }}
            </div>
          </div>
          ...
          <div
            v-if="message"
            class="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-700"
          >
            {{ message }}
          </div>
        </section>
        ...
        >
          ...
          <div
            class="mb-3 flex items-center justify-between gap-2 text-xs text-slate-500"
          >
          ...
          <div
            v-if="!filteredRepairRecords.length"
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  ...
                  <div
                    v-if="record.reviewNote"
                    class="mt-1 text-[0.78rem] leading-[1.5] text-slate-400"
                  >
...
async function fetchRepairRequests() {
  ...
  try {
    const response = await fetch(
      `${GAS_WEB_APP_URL}?action=getRepairRequests&userId=${encodeURIComponent(
